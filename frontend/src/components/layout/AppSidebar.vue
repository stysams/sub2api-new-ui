...
//
// 条目顺序：快速体验 → 密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    { path: '/quick-chat', label: t('nav.quickChat'), icon: QuickChatIcon },
    { path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon },
  )
  if (withDashboard) {
    items.push({ path: '/accounts', label: t('nav.accounts'), icon: GlobeIcon })
  }
  items.push(
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/upstreams', label: t('nav.upstreams'), icon: UpstreamIcon },
    ...
    { path: '/admin/prompt-records', label: t('nav.promptRecords'), icon: ChartIcon },
