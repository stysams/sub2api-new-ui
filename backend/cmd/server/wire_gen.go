...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	coordinator := securityaudit.ProvideCoordinator(legacyEngine, promptService)
	...
	accountVisibilityRepository := repository.NewAccountVisibilityRepository(db)
	accountVisibilityService := service.NewAccountVisibilityService(accountVisibilityRepository, accountRepository)
	accountVisibilityHandler := handler.NewAccountVisibilityHandler(accountVisibilityService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, accountVisibilityHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
